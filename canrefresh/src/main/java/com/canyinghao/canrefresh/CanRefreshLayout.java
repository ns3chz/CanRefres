...
public class CanRefreshLayout extends FrameLayout {
    ...
    private void childLayout() {
        ...
        int paddingBot = getPaddingBottom();
    ...
    private boolean canRefresh() {
        ...
        return !isFooterRefreshing && !isHeaderRefreshing && mRefreshEnabled && mHeaderView != null && !canChildScrollUp();
    }
    ...
    private boolean canLoadMore() {
        ...
        return !isHeaderRefreshing && !isFooterRefreshing && mLoadMoreEnabled && mFooterView != null && !canChildScrollDown();
    }
    ...
    protected boolean canChildScrollUp() {
        ...
        boolean canScrollUp = canScrollUp(mContentView);
        if (canScrollUp) {
            //若能滚动则返回true，否则检查子view能否滚动
            return canScrollUp;
        }
        if (mContentView instanceof ViewGroup) {
            //查找content子view能否滚动
            int contentChildCount = ((ViewGroup) mContentView).getChildCount();
            for (int i = 0; i < contentChildCount; i++) {
                View child = ((ViewGroup) mContentView).getChildAt(i);
                canScrollUp = canScrollUp(child);
                if (canScrollUp) {
                    return canScrollUp;
                }
            }
...
//        }
    ...
    protected boolean canChildScrollDown() {
        ...
        boolean canScrollDown = canScrollDown(mContentView);
        if (canScrollDown) {
            //若能滚动则返回true，否则检查子view能否滚动
        ...
        if (mContentView instanceof ViewGroup) {
            ...
            for (int i = 0; i < contentChildCount; i++) {
                View child = ((ViewGroup) mContentView).getChildAt(i);
                canScrollDown = canScrollDown(child);
                if (canScrollDown) {
                    return canScrollDown;
                }
            }
...
//        }
     ...
     * @param isCoo
     */
    public void setIsCoo(boolean isCoo) {
        this.mIsCoo = isCoo;
    }
